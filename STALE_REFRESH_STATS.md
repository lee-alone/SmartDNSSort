...
为了解决"总查询数不等于缓存命中+缓存未命中"的问题，添加了"缓冲更新"统计项。

## 统计项说明

### 总查询数 = 缓存命中 + 缓存未命中 + 缓冲更新

- **缓存命中 (Cache Hits)**：直接从缓存返回有效数据
- **缓存未命中 (Cache Misses)**：缓存中没有数据，需要向上游查询
- **缓冲更新 (Stale Refresh)**：缓存已过期，但仍返回给用户，同时异步向上游查询更新数据
...
1. **webapi/web/components/dashboard.html**
   - 在"常规统计"卡片中添加"缓冲更新"显示项

2. **webapi/web/js/modules/dashboard.js**
   - 在 `updateDashboard()` 中添加 `cache_stale_refresh` 的显示逻辑

3. **国际化文件**
   - **resources-en.js**：添加 `"cacheStaleRefresh": "Stale Refresh"`
   - **resources-zh-cn.js**：添加 `"cacheStaleRefresh": "缓冲更新"`
